...
class NewReceive():
    ...
    def received(self, content, port):
        # 设置服务器的ip和 port
        # 服务器信息
    ...
    def reveive_model(self, consumer):
        old_prefix = self.read_yaml(path='/home/wuguo-buaa/PycharmProjects/StratosphereLinuxIPS-dev/config.yaml')
        print(old_prefix)
        for message in consumer:
            ...
            if old_prefix >= message.offset:
                ...
                self.set_yaml(path='/home/wuguo-buaa/PycharmProjects/StratosphereLinuxIPS-dev/config.yaml',
                         offset=message.offset)
                if message.value['type'] == 'new_model_k' and message.value['model_host'] == 'k':
                    self.received('new_model', 2234)
                    self.un_gz("/home/wuguo-buaa/PycharmProjects/StratosphereLinuxIPS-dev/modules/flowmldetection"
                               "/model.tar.gz")
                    self.un_tar("/home/wuguo-buaa/PycharmProjects/StratosphereLinuxIPS-dev/modules/flowmldetection"
                                "/model.tar")
                    os.remove("/home/wuguo-buaa/PycharmProjects/StratosphereLinuxIPS-dev/modules/flowmldetection"
                              "/model.tar")
                elif message.value['type'] == 'new_slips_order' and message.value['model_host'] == 'k':
                    if message.value['order_param']['order'] == 'stop':
                        self.p.terminate()
                    else:
                        gui = None
                        ...
                        print(slips_rule)
                        self.p = subprocess.Popen(slips_rule, shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)

                elif message.value['type'] == 'new_slips_conf' and message.value['model_host'] == 'k':
                    self.received('new_conf', 2235)
